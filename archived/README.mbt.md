# Milky2018/mecs archived 0.1

> Archived from commit `a6d8d92`. Package references are adapted from `@mecs`
> to `@archived` so the checked examples remain valid in this subpackage.
> Legacy warnings 20 and 79 are suppressed by the package configuration.
...
Users extend `@archived.Component` and `@archived.Resource`, then implement
...
`@archived.ComponentId::ComponentId("VariantName")` and
`@archived.ResourceId::ResourceId("VariantName")`. The helper functions
...
///|
impl @archived.ComponentValue for ReadmePosition with fn component_id() {
  component_id("ReadmePosition")
}

///|
impl @archived.ComponentValue for ReadmePosition with fn to_component(self) {
  ReadmePosition(self)
}

///|
impl @archived.ComponentValue for ReadmePosition with fn from_component(
  component,
) {
  match component {
...
///|
impl @archived.ComponentValue for ReadmeVelocity with fn component_id() {
  component_id("ReadmeVelocity")
}

///|
impl @archived.ComponentValue for ReadmeVelocity with fn to_component(self) {
  ReadmeVelocity(self)
}

///|
impl @archived.ComponentValue for ReadmeVelocity with fn from_component(
  component,
) {
  match component {
...
///|
impl @archived.ResourceValue for ReadmeFrame with fn resource_id() {
  resource_id("ReadmeFrame")
}

///|
impl @archived.ResourceValue for ReadmeFrame with fn to_resource(self) {
  ReadmeFrame(self)
}

///|
impl @archived.ResourceValue for ReadmeFrame with fn from_resource(resource) {
  match resource {
...
///|
fn readme_move_system(world : @archived.World) -> Unit {
  try! world.each2(fn(
...
///|
fn readme_frame_system(world : @archived.World) -> Unit {
...
test "readme quick start" {
  ...
  let rows : Array[(@archived.EntityId, (ReadmeVelocity, ReadmePosition))] = world
    .query2_entities()
    .to_array()
...
See [the historical basic example](https://github.com/Milky2018/mecs/tree/a6d8d92/examples/basic) for the same pattern as a complete tested
package.
...
See [the historical ergonomics guide](https://github.com/Milky2018/mecs/blob/a6d8d92/docs/ergonomics.md) for copyable component and
resource templates, and [the historical migration guide](https://github.com/Milky2018/mecs/blob/a6d8d92/docs/migration.md) for migration
notes from the earlier experimental designs. Performance benchmarks and storage
tradeoffs are tracked in [the historical performance notes](https://github.com/Milky2018/mecs/blob/a6d8d92/docs/performance.md).
...
```moonbit nocheck
///|
let moving = @archived.QueryFilter().with_component(
...
///|
let stationary = @archived.QueryFilter().without_component(
  _hint=(None : ReadmeVelocity?),
)
